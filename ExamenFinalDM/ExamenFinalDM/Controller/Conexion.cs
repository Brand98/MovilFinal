...
namespace ExamenFinalDM.Controller
{
    public class Conexion
    {
        ...
        {
            var insertar = con.Query<User>("INSERT INTO Usuario VALUES ?,?,?", user, pass, email);
        ...
        {
            string u="", p="", m;
            var search = con.Query<User>("SELECT user, pass FROM Usuario WHERE user = ?, password = ?", user, pass);
